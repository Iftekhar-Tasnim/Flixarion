...
| 10 | As a user, I want to filter content by my accessible sources so that I only see what I can actually watch | Moderate | FR-BROWSE-13 | Given `available_only=true`, then only content with reachable sources for the current user is returned | browse, backend |
